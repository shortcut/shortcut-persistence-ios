...
///     let cat = persistence.get(Cat.self)
///
public protocol ILocalPersistence {
    /// Returns a  storage object by a generic type.
    /// In that case we will return only one element if they will exist. If there will be more
    /// it will be returned first matched item
    ///
    /// - Parameter objectType: Type of object what we want to fetch
    /// - Parameter storage: source of data persistence it can be `keychain`, `sqlite`, `realm`
    ...
    ///
    /// - Parameter objectType: Type of object what we want to fetch
    /// - Parameter storage: source of data persistence it can be `keychain`, `sqlite`
